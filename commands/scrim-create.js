...
module.exports = {
  ...
  async execute(message, args, client, scrims = {}, config = {}, saveData) {
    try {
      ...
      if (args.length < 5) {
        return message.reply('❌ الصيغة: `.scrim create 00:00 00:00 00:00 1/2/3 on/none`');
      }
      ...
      // ✅ ID مبني على الوقت فقط لتجنب التكرار
      ...
      // ❌ إذا السكرم موجود بالفعل بنفس الوقت
      if (diskScrims[scrimId]) {
        return message.reply('❌ فيه سكريم بالفعل بنفس الوقت!');
      }
      ...
      // حفظ السكريم على القرص و الـ cache
      ...
      const registerBtn = new ButtonBuilder()
        .setCustomId(`register_${scrimId}`) // <--- لازم تكون كده بالضبط
        .setLabel('Register')
